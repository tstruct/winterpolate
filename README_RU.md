...
`winterpolate` поддерживает механизмы экранирования, которые часто встречаются в Windows и CI/CD-окружениях.

Обратная кавычка `` ` `` позволяет предотвратить раскрытие Windows-переменной `%VAR%`:
...
Повторная обратная кавычка съедает одну кавычку, сохраняя остальные символы:
...
Последовательность `$$` схлопывается в один `$`. Это позволяет поддерживать распространённый синтаксис экранирования макросов CI/CD:
...
Обратный слеш не используется для экранирования `$`-переменных. Он сохраняется как обычный символ:
...
Это сделано намеренно, чтобы не ломать Windows-пути:
...
* экранирование с помощью обратных кавычек
* `$$`-экранирование для CI/CD
* `NewMapEnv`
* `NewSliceEnv`
...
Этот проект распространяется по лицензии MIT. Подробные условия приведены в файле LICENSE.
